...
private:
  ...
  size_t get_broadcast_index(size_t i, const std::vector<size_t> &other_shape,
                             const std::vector<size_t> &other_stride) const;

  template <Numeric U>
  Tensor<std::common_type_t<T, U>> simd_with_tensor(
      const Tensor<U> &other,
      std::function<void(std::common_type_t<T, U> &, const T &, const U &)>
          callback) const;
  ...
  template <Numeric U>
  Tensor<T> &change_tensor_simd(const Tensor<U> &other,
                                std::function<void(T &, const U &)> callback);

  template <Numeric U>
  Tensor<T> &
  change_tensor_scalar_simd(const U &scalar,
                            std::function<void(T &, const U &)> callback);

public:
  // Constructors

  template <Integral... Dims> Tensor(Dims... dims);
  ...
  template <Numeric U> Tensor(const Tensor<U> &other);
  ...
  template <Numeric U> Tensor<T> &operator=(const Tensor<U> &other);
  ...
  template <Numeric U> Tensor<T> &operator+=(const U &scalar);
  ...
  template <Integral... Indices> TensorView<T> operator()(Indices... indices);
